...
<style>
/* div 默认 flex-direction: row，垂直布局必须显式声明 */
...
- **默认方向**：`div`、`scroll` 等容器组件默认 `flex-direction: row`；页面内容垂直排列时，**必须**在根组件样式中显式添加 `flex-direction: column`
- **单位省略**：数字默认为 px，无需写单位
...
- [ ] 样式使用 Flexbox
- [ ] 页面内容垂直排列时，根组件样式已显式添加 `flex-direction: column`（默认是 row）
